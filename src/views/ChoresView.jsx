        ...
        <>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8, marginBottom: 14 }}>
            ...
            <div style={{ background: '#0a100a', border: '1px solid #1e5a3a', borderRadius: 10, padding: '10px 8px', textAlign: 'center' }}>
              ...
              <div style={{ color: '#06b6d4', fontWeight: 700, fontSize: '1rem' }}>${(khanaData?.entries && khanaData.entries.length > 0 ? (khanaData.entries[0]?.rewardEarned || 0) : 0).toFixed(2)}</div>
              <div style={{ color: '#2a5a5a', fontSize: '0.65rem', marginTop: 2 }}>{khanaData?.entries && khanaData.entries.length > 0 ? `${khanaData.entries[0]?.totalMinutes || 0}m / ${khanaData.entries[0]?.targetMinutes || 0}m` : '0m / 0m'}</div>
            </div>
